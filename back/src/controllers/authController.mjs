...
import ModelCadastro from "../models/modelAuth.mjs";


const loginUsuario = (req, res) => {
  const {email, senha} = req.body;
  ModelCadastro.loginUsuario(email, senha, (err, resultado) => {
    if (err) {
...
const cadastroUsuario = (req, res) => {
  ...
  ModelCadastro.cadastroUsuario(nome, email, senha, dt_criacao, (err, resultado) => {
    if (err) {
...
const buscaCadastro = (req, res) => {
  ...
  ModelCadastro.buscaCadastro(codigo, (err, resultados) => {
    if (err) {
...
const vincCadastro = (req, res) => {
  ...
  const { nome, email, senha, cod_casal, dt_criacao} = req.body;

  ModelCadastro.vincCadastro(nome, email, senha, cod_casal, dt_criacao, (err, resultado) => {
    if (err) {
